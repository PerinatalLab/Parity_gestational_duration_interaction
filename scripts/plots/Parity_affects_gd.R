...
m16 = lmer(GRDBS ~ Parity_logreg+ KON + AR + AR2 +(1 | lpnr_mor), data=dat_m2, control = lmerControl(optimizer ="bobyqa"))
betam16=beta_CI(m16, 0.025,0.975)

m17 = lmer(GRDBS ~ Parity_logreg+ KON + AR + AR2 + mor_birth_country_NORDIC + MALDER + MALDER2  + as.numeric(max_grade_mor) + as.numeric(max_grade_far) +(1 | lpnr_mor), data=dat_m2,control = lmerControl(optimizer ="bobyqa"))
...
#### Saving ####
#ggsave("/home/karin/Parity_Project_gd/scripts/plots/output_test/plot.eps",p, width = 174, height = 140, units = "mm", bg = "transparent", device=cairo_ps)
#ggsave("/home/karin/Parity_Project_gd/scripts/plots/output_test/plot.png",p, width = 174, height = 140, units = "mm")
#fwrite(model_info_final,"~/Parity_Project_gd/scripts/plots/output_test/model_info.csv",sep=",")
#fwrite(plotdata, "~/Parity_Project_gd/scripts/plots/output_test/plotdata.csv", sep=",")
