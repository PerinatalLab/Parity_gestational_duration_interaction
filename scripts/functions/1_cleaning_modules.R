...
fun_dadID = function(dat) {
  n_row_before  = nrow(dat)
  cat("Father IDs:
      - before the cleaning (first character of personal number):")
  print(table(substr(dat$lpnr_far,1,1),useNA="a"))
  dat = dat[which(dat$lpnr_far>0),] # needed for twin-removal (remaining 4 073 802)
  n_row_after = nrow(dat)
  cat("
      - after the cleaning (first character of personal number):")
  print(table(substr(dat$lpnr_far,1,1),useNA="a"))
  ...
  generate_year_counts(dat, "farID", F)
  dat
}
...
- number of unique mom-child pairs that are entered more than once:", length(dpl_codes))
        ...
        cols_to_use = which(colnames(tmp0_dpl) %in% c("AR","GRDBS","PARITET","lpnr_BARN","lpnr_mor"))
    ...
    running pruning scipt which leaves only one row per mom-child pair: the row with most info")
        ...
        for (b_id in unique(tmp0_dpl$lpnr_BARN)) {
                ...
                sub = sub[which(!is.na(sub$GRDBS)), ] # strictly must be present
...
fun_mBmiQC = function(dat) {
        ...
        # remove bad rows
        cat("\t",length(bad_row_indx),"Number of BMIs that will be set as NA \n")
        #if(length(bad_row_indx)>0) dat=dat[-bad_row_indx,]
        #cat("\n rows remaining: ", nrow(dat))        
        
	# Put bad rows as NA
	dat$BMI[bad_row_indx] <- NA	
        ...
        # restore original order
        dat = dat[order(dat$original_order),]
        dat = as.data.frame(dat)[,orig_clnms]
        rm(orig_clnms)
        return(dat)
}
