...
dat = dat %>% filter(parity_clean<=4)   # need it by parity and not grouped after parity 3 - remove all preg above parity 3. 
...
#### Step 2 - Extracting the spontaneous birth ####
# Adding the functions from scripts to the enviroment
...
## Whole population
dat_m21 = filter(dat_m2, parity_clean>1) # The mother need to have had a pregnancy to have a history of preterm delivery
dat_m21 = group_by(dat_m21, lpnr_mor) %>% filter(n()>1) # Mother need to have at lest two pregnancies
dat_m22 = dat_m21 %>% group_by(lpnr_mor) %>% filter(!is.na(prev_PTD)) %>% filter(n()>1) # Mother need to have at lest two pregnancies
...
betam1_lm=beta_CI(m1, 0.025,0.975)
print(summary(m1))
...
## Interaction parity*prio history of preterm delivery
beta_CI = function(Model, CI_min, CI_max) {
  m =character()
  for (i in c(2,13:14)) {
          mm = summary(Model)$coefficients[i,1]
  ...
  rownames(m) = c("obshist","Parity_logreg3:obshist","Parity_logreg4:obhist")
  colnames(m) = c("CI_min","Beta","CI_max")
...
betamI1_lm=beta_CI(mI1, 0.025,0.975)
print(summary(mI1))
...
#### Saving ####
#ggsave("/home/karin/Parity_Project_gd/plots/obstetric_history.png",p, width = 174, height = 105, dpi = 1200, units = "mm", device='png')
ggsave(snakemake@output[[1]],p,width = 174, height = 105, dpi = 1200, units = "mm", bg = "transparent", device=cairo_ps)
...
#fwrite(as.data.frame(model_info_final),"/home/karin/Parity_Project_gd/plots/clinical_history_model_info.csv",sep=",")
fwrite(model_info_final,snakemake@output[[2]],sep=",")

#fwrite(as.data.frame(plotdata),"/home/karin/Parity_Project_gd/plots/clinical_history_plotdata.csv", sep=",")
fwrite(as.data.frame(plotdata),snakemake@output[[3]], sep=",")




#### Check, for suplementary material ####

dat_m22 = dat_m22 %>% group_by(lpnr_mor) %>% arrange(parity_clean) %>% mutate(parity_ptd = dplyr::lag(prev_PTD)) %>% mutate(parity2_ptd = ifelse(Parity_logreg==4, parity_ptd, NA)) # preterm delivery in the second pregnancy (ptd in parity 1 for the model m2p4 below)
dat_m22$parity2_ptd = factor(dat_m22$parity2_ptd)
...
beta_CI = function(Model, CI_min, CI_max) {
  ...
  rownames(m) = c("PrevPTD","firstbornPTD")
  colnames(m) = c("CI_min","Beta","CI_max")
  return(m)
}


m2p3 = lm(GRDBS ~  prev_PTD +  KON + AR + AR2 + mor_birth_country_NORDIC + MALDER + MALDER2 + as.numeric(max_grade_mor) + as.numeric(max_grade_far) + PTB_first_born, data = filter(dat_m22, parity_clean ==3))
betam2p3=beta_CI(m2p3, 0.025,0.975)
...
beta_CI = function(Model, CI_min, CI_max) {
  ...
  rownames(m) = c("PrevPTD","firstbornPTD","parity1PTD")
  colnames(m) = c("CI_min","Beta","CI_max")
  return(m)
}


m2p4 = lm(GRDBS ~  prev_PTD +  KON + AR + AR2 + mor_birth_country_NORDIC + MALDER + MALDER2 + as.numeric(max_grade_mor) + as.numeric(max_grade_far) + PTB_first_born + parity2_ptd, data = filter(dat_m22, parity_clean ==4))
betam2p4=beta_CI(m2p4, 0.025,0.975)
